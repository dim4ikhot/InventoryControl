...
uses U_DM, U_Main, FIBDataSet,U_MessageCP;
...
begin
  ...
  begin
    ...
    currentUser.UFullName := DM.Autority_Table.FieldValues['Name'];
    currentUser.UName := DM.Autority_Table.FieldValues['UserName'];
    currentUser.UIsAdmin := Boolean(DM.Autority_Table.FieldValues['IsAdmin']);
    if DM.Autority_TableSTOCK_ID.AsString <> '' then
      currentUser.userStockId := DM.Autority_Table.FieldValues['STOCK_ID'];
    Close;
