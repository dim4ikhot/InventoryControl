...
begin
  ...
  try
    ...
    // <2002-11-17> dllee
    // §?·s Dimension ??¦b wirteEOF ¤§«??A¦]¬°¦b¦? if ¤???¤F Seek §??? position
    // if r > xls.maxrows then begin
    //   xls.maxrows:=r+1;
    //   xls.fstream.Seek(10,soFromBeginning);
    //   xls.WriteDimension;
    // end;
    // ¦?¦]¬°¤w±N maxrows ?]¬°??¤j­??A¦??B¦?®?¦??N?u??¦? 65535?A©??H?A¤?¦A§P?_?C
  finally
    xls.free;
  end;
end;
...
begin
  ...
  if rMax > xls.maxrows then          // ¦?®?¦???¦h?u??¦s 65535 Rows
    rMax:=xls.maxrows;
  try
    xls.writeBOF;
    xls.WriteDimension;
...
begin
  ...
  maxCols:=100;   // <2002-11-17> dllee Column ????¬O¤??i??¤j©? 65535, ©??H¤?¦A?B?z
  maxRows:=65535; // <2002-11-17> dllee ?o­?®?¦???¤j?u???o»?¤j?A???`·N¤j????®?®w«?®e©??N¤j©??o­?­?
end;
...
begin
  case field.DataType of
     ...
     ftFloat, ftBCD:
       ...
       Cellstr(vcol,vrow,EmptyStr);   // <2002-11-17> dllee ???L«¬?A?g¤J????¦r¦?
  end;
end;
