...
begin
  try
    ...
    begin
      if (not (DM.tableProducts.Locate('KOD',ProductCode.Text, [])))or
      ((DM.tableProducts.Locate('KOD',CBNames.Text, []))and
         (DM.tableProductsPRICE.AsFloat <> ProductPrice.Value)) then
      begin
        try
         //TODO: Move This code to afterPrint Invoice In
          ...
          DM.tableProductsTOTALPRICE.AsFloat := StrToFloatDef(ProductTotalPrice.Text,0);
          ...
          DM.tableInvoiceInTOTALPRICE.AsFloat := StrToFloatDef(ProductTotalPrice.Text,0);
          ...
          dm.mtAddProductsproductStock.AsInteger := StrToIntDef(CBAllStocks.Values[CBAllStocks.ItemIndex-1],0);
      ...
      begin
        if ShowMessagerCP(LangNewProduct.GetText('EmptyFieldCapt'),
                              LangNewProduct.GetText('ProductExists'),mtWarning,[mbYes,mbNo]) = 6 then
        begin
          DM.tableProducts.Edit;
          DM.tableProductsSTOCK_ID.AsInteger := StrToIntDef(CBAllStocks.Values[CBAllStocks.ItemIndex-1],0);
          DM.tableProductsMEASURE.AsString := ProductMeasured.Text;
          DM.tableProductsKOLVO.AsInteger := DM.tableProductsKOLVO.AsInteger + ProductCount.IntValue;
          DM.tableProductsPRICE.AsFloat := ProductPrice.Value;
          // TODO:  ProductName.Text change to ID
          DM.tableProductsNAME_ID.AsString := ProductName.Text;
          DM.tableProductsKOD.AsString := ProductCode.Text;
          DM.tableProductsTOTALPRICE.AsFloat := DM.tableProductsTOTALPRICE.AsFloat +
                                                               StrToIntDef(ProductTotalPrice.Text,0);
